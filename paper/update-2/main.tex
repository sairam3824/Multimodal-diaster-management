...
\textbf{IoT sensor data.} We compiled 63,527 samples from six sources: California Wildfire records (14,988 rows), Historical Tropical Storm Tracks (59,228), Atlantic Storms (22,705), Global Earthquakes (8,394), Iran Earthquakes (52,268), and Sri Lanka Flood Risk (25,000). The Sri Lanka Flood Risk dataset is a synthetic dataset designed for prototyping flood risk assessment pipelines~\cite{srilanka_flood}; we include it to ensure balanced class coverage but note that the perfect IoT F1 scores on flood and related categories are partially attributable to this synthetic data and may not generalize to noisy operational sensor deployments. After preprocessing, class support: fire (4,971), storm (13,162), earthquake (18,394), flood (25,000), unknown (2,000). All features are encoded into a unified 32-dim vector with cyclic time encoding (sin/cos for month and hour).
...
\end{tabular}
\vspace{2pt}
\raggedright\scriptsize{$^\dagger$Perfect F1 scores for storm, earthquake, and flood are partially attributable to the use of synthetic sensor data with well-separated feature distributions. Performance on noisy real-world sensor deployments may be lower.}
\end{table}

The model achieves perfect classification (F1 = 1.0) for storm, earthquake, and flood categories, though as noted, the synthetic nature of portions of the IoT training data contributes to these results. The fire--unknown confusion stems from overlapping weather-group features. Severity regression achieves MAE = 0.0452 and $R^2 = 0.745$.
...
Despite the segmentation performance gap, the satellite sub-model's primary role in our architecture is to produce discriminative 640-dim embeddings for the fusion layer, not standalone segmentation maps. The ablation study (Table~\ref{tab:ablation}) demonstrates that even with current segmentation performance, the satellite embeddings contribute a \textbf{+29.8 percentage point} improvement in priority accuracy and \textbf{64.4\% reduction} in severity MAE when added to the crisis-only baseline---confirming that the embeddings capture meaningful damage information for downstream fusion despite the per-pixel metric gap.
...
\textbf{Synthetic data reliance.} The IoT sub-model's training data includes the Sri Lanka Flood Risk dataset, which is synthetic. While this provides balanced class coverage, the perfect F1 scores (1.0) for storm, earthquake, and flood categories should be interpreted with caution---real-world sensor data exhibits noise, drift, and missing values that synthetic data does not capture. Validation on operational sensor deployments is needed before production use.
...
\begin{enumerate}
    ...
    \item \textbf{Operational IoT validation}: Replacing synthetic IoT data with real sensor deployments (e.g., USGS seismic networks, NOAA weather stations) to validate model robustness under real-world noise conditions.
...
\bibitem{srilanka_flood}
``Sri Lanka Flood Risk Dataset,'' Kaggle, 2023. [Online]. Available: https://www.kaggle.com/datasets/srilanka-flood-risk. Note: This is a synthetic dataset designed for prototyping flood risk assessment models.
