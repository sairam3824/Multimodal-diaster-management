...
The proposed architecture processes three independent data streams through specialized sub-models, producing fixed-dimensional embeddings that are fused through pairwise cross-attention with adaptive gating. Fig.~\ref{fig:trifusion_architecture} illustrates the complete pipeline.
...
To address the severe class imbalance inherent in xBD (no-damage pixels vastly outnumber damage classes), we employ a class-weighted CrossEntropy loss:

\begin{equation}
\mathcal{L}_\text{seg} = -\sum_{c=1}^{C} w_c \, y_c \log(\hat{y}_c)
\end{equation}

where $w_c$ are per-class weights. In our final GPU-accelerated configuration, uniform class weights ($w_c = 1.0$ for all classes) were found to perform well when combined with an extensive data augmentation pipeline and cosine annealing schedule—the augmentation diversity and learning rate scheduling provided sufficient implicit regularization against class imbalance. The model is trained with an extensive data augmentation pipeline including horizontal/vertical flips, random 90$^\circ$ rotations, shift-scale-rotate, brightness-contrast adjustment, and Gaussian noise injection. Gradient clipping at 1.0 ensures stable training dynamics.
...
We employ a differential learning rate strategy: the pretrained ResNet101 encoder is trained at $3 \times 10^{-5}$ while the decoder operates at $1 \times 10^{-4}$ and projection heads at $2 \times 10^{-4}$. This prevents catastrophic forgetting of ImageNet-pretrained features while still allowing domain adaptation for satellite imagery. A cosine annealing schedule smoothly decays learning rates over the 30-epoch training run.
...
\textbf{Satellite imagery.} The xBD dataset~\cite{gupta2019xbd} with post-disaster satellite images ($512 \times 512$) across 4 damage classes (no-damage, minor, major, destroyed) spanning 12 disaster events (hurricanes, earthquakes, wildfires, flooding, tsunami, and volcanic eruptions). After filtering and preprocessing, 2,684 samples were used with an 80/20 train/val split (2,147 train, 537 val).
...
All models are implemented in PyTorch. Training details for each sub-model are summarized in Table~\ref{tab:training_config}. The IoT, crisis, and xBD sub-models were trained on Kaggle GPU instances (NVIDIA Tesla P100, 16\,GB), while the tri-fusion layer was trained on CPU. The xBD model was retrained on GPU in the final iteration, achieving improved convergence in 30 epochs (1.5 hours) compared to prior CPU-based training runs.
...
Device & P100 GPU & P100 GPU & Kaggle GPU & CPU \\
\bottomrule
\end{tabular}
\end{table}
...
The modified DeepLabV3+ with class-weighted CrossEntropy loss achieves best validation Mean IoU of \textbf{0.4276} at epoch 27 (out of 30), with Mean F1 of \textbf{0.5221}. This represents a \textbf{52.6\% improvement} in Mean IoU and \textbf{50.1\% improvement} in Mean F1 compared to our initial configuration (mIoU = 0.2802, 32 epochs on CPU), achieved through GPU-accelerated training on a Kaggle NVIDIA Tesla P100, a cosine annealing learning rate schedule, and an extensive data augmentation pipeline. The GPU-accelerated training completed 30 epochs in approximately 1.5 hours (3.0 minutes per epoch), enabling faster iteration and improved convergence. Table~\ref{tab:xbd_results} shows per-class results.
...
Our Mean IoU of 0.4276 represents a substantial improvement over our initial CPU-trained configuration (mIoU = 0.2802), achieved through GPU-accelerated training on a Kaggle NVIDIA Tesla P100 with cosine annealing scheduling and an extensive augmentation pipeline. GPU training enabled faster convergence in 30 epochs (1.5 hours total) compared to our prior 60-epoch CPU run, while improving all per-class metrics. However, the result remains below the current state of the art on xBD, where optical-only methods achieve 66--70\% mIoU~\cite{benson2025xbdsota}. We attribute the remaining gap to two primary factors:

\textbf{(1) Class imbalance severity.} The xBD dataset exhibits extreme class imbalance: no-damage pixels comprise $>$85\% of annotated regions. Our No-Damage IoU (0.9774) is near-perfect, but minority damage classes remain challenging (Minor: 0.1595, Major: 0.2647, Destroyed: 0.3087). Competitive approaches additionally employ class-frequency-based oversampling, copy-paste augmentation of damaged buildings, and multi-scale training with higher-resolution crops~\cite{benson2025xbdsota}.

\textbf{(2) Single-scale inference.} Our model performs single-scale inference at $512 \times 512$ resolution, whereas top-performing methods use multi-scale test-time augmentation with horizontal and vertical flips, which typically adds 2--5\% mIoU on the xBD benchmark.

\textbf{Embedding-centric design rationale.} Despite the remaining segmentation gap on minority classes, the satellite sub-model's primary role in our architecture is to produce \emph{discriminative 640-dim embeddings} for the fusion layer, not standalone segmentation maps. The ablation study (Table~\ref{tab:ablation}) demonstrates that the satellite embeddings contribute a \textbf{+29.8 percentage point} improvement in priority accuracy and \textbf{64.4\% reduction} in severity MAE when added to the crisis-only baseline---confirming that the embeddings capture meaningful damage information for downstream fusion. The model need not produce pixel-perfect segmentation to extract features that distinguish ``severe structural damage'' from ``minor cosmetic damage'' at the embedding level.

\begin{figure}[!t]
\centering
\includegraphics[width=\columnwidth]{xbd_training_history.png}
\caption{xBD model training history (Kaggle GPU, 30 epochs). Top-left: train/val loss convergence. Top-right: validation Mean IoU reaching best at epoch 27 (0.4276). Bottom-left: Mean F1 progression. Bottom-right: cosine annealing learning rate schedule.}
...
Our xBD satellite model (mIoU = 0.4276) narrows the gap with state-of-the-art segmentation methods (66--70\% mIoU), representing a 52.6\% improvement over our initial configuration (mIoU = 0.2802) through GPU-accelerated training with cosine annealing and an extensive augmentation pipeline. As discussed in Section~VI-C, the remaining gap is primarily attributable to class imbalance severity and single-scale inference. Crucially, the satellite embeddings provide the largest single-modality contribution to fusion performance (+29.8pp priority accuracy), confirming that discriminative embeddings for cross-modal reasoning do not require pixel-perfect segmentation.
...
\textbf{Partial CPU training.} While the IoT, crisis, and xBD sub-models were trained on Kaggle GPU instances (NVIDIA Tesla P100), the tri-fusion layer was trained on CPU due to hardware scheduling constraints. The xBD model was retrained on GPU in the final iteration, improving mIoU from 0.2802 to 0.4276 over 30 epochs (1.5 hours). Further improvements through larger batch sizes (16--32), copy-paste augmentation, and multi-scale training are expected to close the remaining gap with state-of-the-art methods.
...
\textbf{xBD segmentation gap.} While the satellite model's overall mIoU improved to 0.4276, per-pixel segmentation performance on minority damage classes (Minor: 0.1595, Major: 0.2647, Destroyed: 0.3087) remains below production thresholds. The embeddings are effective for fusion, but operational deployment requiring standalone damage mapping would benefit from the improvements outlined in Section~\ref{sec:future}.
...
\begin{enumerate}
    \item \textbf{Advanced xBD training techniques}: Building on our GPU-retrained model (mIoU = 0.4276), further improvements through larger batch sizes (16--32), extended training (100+ epochs), multi-scale training and test-time augmentation, and copy-paste augmentation for minority damage classes. GPU-accelerated training improved mIoU from 0.2802 to 0.4276; further optimization with larger batch sizes, copy-paste augmentation, and multi-scale training is expected to unlock additional gains. We target $>$0.55 mIoU on damage classes toward closing the gap with state-of-the-art methods (66--70\% mIoU).
