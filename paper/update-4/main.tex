...
where $\mathcal{L}_\text{Dice} = 1 - \frac{2\sum_i p_i g_i + \epsilon}{\sum_i p_i + \sum_i g_i + \epsilon}$ is computed per-class and averaged, and $\mathcal{L}_\text{Focal} = -\alpha_t (1-p_t)^\gamma \log(p_t)$ with $\gamma=2$ and effective-samples-based $\alpha_t$ weights. We set $\lambda_D = 0.5$ and $\lambda_F = 0.5$. Class weights are computed using the effective number of samples~($\beta=0.999$) to inversely weight each class by its pixel frequency, mitigating the tendency of standard cross-entropy to be dominated by the majority no-damage class. The model is additionally trained with an extensive data augmentation pipeline including horizontal/vertical flips, random 90$^\circ$ rotations, shift-scale-rotate, brightness-contrast adjustment, and Gaussian noise injection. Gradient clipping at 1.0 ensures stable training dynamics.
...
We employ a differential learning rate strategy: the pretrained ResNet101 encoder is trained at $3 \times 10^{-5}$ while the decoder operates at $1 \times 10^{-4}$ and projection heads at $2 \times 10^{-4}$. This prevents catastrophic forgetting of ImageNet-pretrained features while still allowing domain adaptation for satellite imagery. An initial 5-epoch warmup linearly ramps the learning rates from 0 to their target values before cosine annealing begins.
...
Encoder LR & --- & --- & 3e-5 & --- \\
LR warmup & --- & --- & 5 epochs & --- \\
Scheduler & Cosine & CosineWR & Warmup+Cosine & Cosine \\
...
The modified DeepLabV3+ with Dice + Focal loss and effective-samples class weighting achieves best validation Mean IoU of \textbf{0.4155} at epoch 14 (out of 60), with Mean F1 of \textbf{0.5085}. This represents a \textbf{48.3\% improvement} in Mean IoU and \textbf{46.2\% improvement} in Mean F1 compared to our earlier configuration (mIoU = 0.2802, 32 epochs), achieved through extended training (60 epochs), effective-samples-based class weighting ($\beta = 0.999$), a 5-epoch learning rate warmup, higher encoder learning rate ($3 \times 10^{-5}$ vs.\ $1 \times 10^{-5}$), and an extensive data augmentation pipeline. Despite CPU-only training constraints, these hyperparameter and loss refinements yielded substantial gains. Table~\ref{tab:xbd_results} shows per-class results.

\begin{table}[!t]
\centering
\caption{xBD Satellite Model Per-Class Results (Best Checkpoint, Epoch 14)}
\label{tab:xbd_results}
\begin{tabular}{lcc}
\toprule
\textbf{Class} & \textbf{IoU} & \textbf{F1-Score} \\
\midrule
No-Damage & \textbf{0.9704} & \textbf{0.9849} \\
Minor-Damage & 0.1541 & 0.2587 \\
Major-Damage & 0.2390 & 0.3604 \\
Destroyed & 0.2985 & 0.4297 \\
\midrule
\textbf{Mean} & \textbf{0.4155} & \textbf{0.5085} \\
\bottomrule
\end{tabular}
\end{table}
...
Our Mean IoU of 0.4155 represents a substantial improvement over our initial configuration (mIoU = 0.2802) through extended CPU training (60 vs.\ 32 epochs), effective-samples class weighting, a 5-epoch warmup schedule, and an extensive augmentation pipeline. However, it remains below the current state of the art on xBD, where optical-only methods achieve 66--70\% mIoU~\cite{benson2025xbdsota}. We attribute the remaining gap to three primary factors:

\textbf{(1) CPU-only training.} The xBD satellite model was trained on CPU, which constrained batch size to 4 and limited hyperparameter exploration throughput. State-of-the-art methods typically train on GPU with batch sizes of 16--32, enabling more extensive augmentation-heavy pipelines and faster convergence. Despite these constraints, our optimized training recipe (effective-samples weighting, warmup + cosine schedule, higher encoder learning rate) closed 48.3\% of the gap relative to our earlier configuration.

\textbf{(2) Class imbalance severity.} The xBD dataset exhibits extreme class imbalance: no-damage pixels comprise $>$85\% of annotated regions. Our No-Damage IoU (0.9704) is near-perfect, but minority damage classes remain challenging (Minor: 0.1541, Major: 0.2390, Destroyed: 0.2985). Competitive approaches additionally employ class-frequency-based oversampling, copy-paste augmentation of damaged buildings, and multi-scale training with higher-resolution crops~\cite{benson2025xbdsota}.
...
\caption{xBD model training history (CPU, 60 epochs, first 18 shown). Top-left: train/val loss convergence. Top-right: validation Mean IoU reaching best at epoch 14 (0.4155). Bottom-left: Mean F1 progression. Bottom-right: warmup + cosine annealing learning rate schedule.}
...
Our xBD satellite model (mIoU = 0.4155) narrows the gap with state-of-the-art segmentation methods (66--70\% mIoU), representing a 48.3\% improvement over our initial configuration (mIoU = 0.2802) through extended CPU training with optimized hyperparameters---effective-samples class weighting, 5-epoch warmup, and higher encoder learning rate. As discussed in Section~VI-C, the remaining gap is primarily attributable to CPU-only training constraints, class imbalance, and single-scale inference. Crucially, the satellite embeddings provide the largest single-modality contribution to fusion performance (+29.8pp priority accuracy), confirming that discriminative embeddings for cross-modal reasoning do not require pixel-perfect segmentation.
...
\textbf{Partial CPU training.} While the IoT and crisis sub-models were trained on a Kaggle P100 GPU, the xBD satellite model and tri-fusion layer were trained on CPU due to hardware scheduling constraints. Despite CPU constraints, optimized hyperparameters (effective-samples class weighting, 5-epoch warmup + cosine schedule, encoder LR of $3 \times 10^{-5}$, and extensive augmentation) improved xBD mIoU from 0.2802 to 0.4155 over 60 epochs with batch size 4. GPU-accelerated retraining with larger batch sizes (16--32) and advanced techniques (copy-paste augmentation, multi-scale training) is expected to further close the gap with state-of-the-art methods.
...
\textbf{xBD segmentation gap.} While the satellite model's overall mIoU improved to 0.4155, per-pixel segmentation performance on minority damage classes (Minor: 0.1541, Major: 0.2390, Destroyed: 0.2985) remains below production thresholds. The embeddings are effective for fusion, but operational deployment requiring standalone damage mapping would benefit from the improvements outlined in Section~\ref{sec:future}.
...
\begin{enumerate}
    \item \textbf{GPU-accelerated xBD retraining}: Retraining the satellite model on GPU with larger batch sizes (16--32), extended training (100+ epochs), multi-scale training and test-time augmentation, and copy-paste augmentation for minority damage classes. Our CPU-only training already improved mIoU from 0.2802 to 0.4155 through hyperparameter optimization alone; GPU acceleration is expected to unlock further gains. We target $>$0.55 mIoU on damage classes toward closing the gap with state-of-the-art methods (66--70\% mIoU).
