...
Our proposed \textbf{Tri-Modal Fusion} framework addresses this issue. It combines three data streams: environmental metadata, satellite images of damage, and image/text posts from social media into one architecture by utilizing learned cross-modal attention with adaptive gating. The key difference between our framework and previous multi-modal models is that this framework does not necessarily require all three inputs for operation.

\textbf{Terminology.} We use ``IoT'' throughout as shorthand for the environmental and seismological metadata sub-model, but it is worth being upfront: the training data here comes from historical weather records, seismological catalogs, storm track databases, and a synthetic flood risk dataset---not from live IoT sensor feeds like MQTT or LoRaWAN streams. That said, the architecture itself is sensor-agnostic. The 32-dimensional input vector and the adaptive confidence weighting could plug directly into a real-time sensor deployment. We return to this distinction in Section~\ref{sec:limitations}.
...
\begin{enumerate}
    ...
    \item \textbf{Noise Sensitivity Testing}: For performing a stress test on the head of the IoT, we subject it to 5\%, 10\%, and 20\% Gaussian noise, respectively, in the inference phase. It can be seen that the accuracy drops gradually rather than abruptly, with this trend continuing as we introduce even more noise levels that have not been seen by our model during training.
 
    \item \textbf{Ablation Testing}: We perform an ablation test for all possible modality configurations, and we show that adding each successive data source improves all metrics significantly.
\end{enumerate}
...
\textbf{Social Media Analysis.} Among the early attempts of multimodal benchmarking for crisis content was the dataset called CrisisMMD~\cite{alam2018crisismmd}. Alam et al. showed that simply fusing the image and textual features increased humanitarian classification performance; however, their approach of feature fusion only involved concatenation of features. CrisisNLP~\cite{imran2015processing} and CrisisLex~\cite{olteanu2015crisislex} were two unimodal datasets. The use of attention-based fusion methods for crisis tweets was studied by Abavisani et al.~\cite{abavisani2020multimodal}, which did not include sensors and satellite data. The current best results on CrisisMMD are due to Mouzannar et al.~\cite{mouzannar2025guidedcross}, who achieved a success rate between 93.92\% and 94.02\% using the fusion technique called Guided Cross-Attention on LLaVA-generated captions.

\textbf{Damage Assessment Using Satellites.} xBD~\cite{gupta2019xbd} is currently the largest benchmark in the area of assessing building damage from pre-disaster/post-disaster satellite images. The best xView2 challenge models~\cite{xview2} relied on U-Nets and semantic segmentation neural nets. Weber and Kan~\cite{weber2020building} tried different networks focused on change detection in satellite imagery. Using purely optical approaches, Mean IoU on xBD has been boosted to 66--70\% via Dice+Focal loss, multi-scale features, and heavy augmentations~\cite{benson2025xbdsota}.
...
where $w_c$ is class weighting factor. However, surprisingly, setting class weight uniformly to 1.0 seemed sufficient enough for the GPU-based implementation since some form of regularization due to the introduction of augmentation techniques such as cosine annealing appeared to offset the impact of class imbalance problem. Apart from sampling patches from images, we perform augmentation of our data with methods such as flips, rotation by 90$^\circ$, shift-scale-rotate augmentation, change of image's brightness and contrast, as well as addition of small Gaussian noise. The gradients are clipped at 1.0 to avoid unstable training behavior.

\subsubsection{Satellite Embedding ($\mathbf{F}_\text{sat}$)}

In this case, the final feature map that the encoder produces will have dimensions of $2048 \times 16 \times 16$. We apply downsampling to produce 2048D vector features and then project it to 512D space.
...
\subsection{Quad-stack XAI Fallback Pipeline}

There is no such thing as a perfect XAI methodology. Weight drift, gradient explosion/vanishing, architecture changes etc. can occur in XAI. So we account for that by switching to another fallback method if something goes wrong:

\begin{enumerate}
    \item \textbf{First}: Gradient-Weighted Attention Rollout (Fallback to best quality, primary)
    \item \textbf{Second}: Attention Rollout without gradients (Fallback if gradients cannot be calculated)
    \item \textbf{Third}: Input Gradients from Cosine Saliency Map (Fallback if attention maps cannot be used)
    \item \textbf{Fourth}: Zero array creation (``nothing'')
\end{enumerate}

Simplified: We do not want to give back nothing, even though it could potentially be more accurate.

\subsection{Lockstep Image/Text XAI}

This technique offers two explanations simultaneously: the Grad-CAM image heat-map, together with the briefing by GPT-4o based on the text prompt. In other words, the person in the loop receives both the attention spatial map of what the machine focused on, plus the textual instructions with actionable advice. Crucially, it must be emphasized that the GPT-4o does not overwrite the machine's prediction, it merely offers description and embellishment.
...
\midrule
\multicolumn{3}{l}{\textit{Methods we did not explicitly compare:}} \\
Mouzannar et al.~\cite{mouzannar2025guidedcross} & 93.92--94.02\% & 2025 \\
\bottomrule
\end{tabular}
\vspace{2pt}
\raggedright\scriptsize{For Mouzannar et al., captions from images are automatically generated via LLaVA using their Guided Cross Attention method. They have a completely different architecture as well; theirs is not meant for single-task fine-tuning but multimodal fusion.}
\end{table}

\begin{figure}[!t]
\centering
\includegraphics[width=\columnwidth]{crisis_training_history.png}
\caption{ImageNet Training Curves. Training ResNet-50 on crisis models on ImageNet for 10 epochs. The graph on the left shows the validation accuracy flattening out to a maximum of 87.27\%. This confirms that early stopping is employed to choose the best model. In the right graph, the weights assigned to vision and text using adaptive modality confidence ($\sim$0.50 for vision, $\sim$0.45 for text) are roughly balanced throughout the training process.}
...
\textbf{Why this is fine for us.} Standalone segmentation is not the satellite sub-model's real job in this architecture---its job is to hand back \emph{discriminative 640-dim embeddings} to the fusion layer. At that, it does well. Table~\ref{tab:ablation} shows that bolting satellite embeddings onto the crisis-only baseline lifts priority accuracy by \textbf{+29.8 percentage points} and cuts severity MAE by \textbf{64.4\%}. Pixel-perfect masks are not required to produce embeddings that can separate ``severe structural collapse'' from ``minor cosmetic damage.'' GPU training with large batch sizes and multi-scale prediction should reduce the segmentation gap in future iterations.
...
Ablation studies form the crux of this study and provide a detailed analysis of the contribution of each modality. Table~\ref{tab:ablation} shows four different setups, all evaluated using the same 2,722 validation samples.
...
\textbf{1) Satellite is most influential.} Using only the satellite channel reduces the Severity MAE by 64.4\% (0.1165 $\to$ 0.0415) and boosts Priority Accuracy by 29.8\% (68.96\% $\to$ 98.75\%). This can be easily attributed to the physical representation of damage from satellite images, something other channels cannot achieve.

\textbf{2) IoT still makes sense.} Adding an IoT layer reduces the Severity MAE by 15.1\% and improves Priority Accuracy by 3.4\%. The IoT layer provides information about the environment including weather conditions, seismic activity, and hydrological conditions, something that a picture-only view cannot accomplish.

\textbf{3) Three modalities are better than two.} A complete trifusion network outperforms the best two-modality configuration (crisis+satellite) by 4.1\% in severity MAE and 0.66pp in priority accuracy. Every modality adds value to the system.

\textbf{4) Degradation is monotonic.} Metrics gradually improve when modalities are sequentially added. Even the worst-case scenario where only the crisis modality is used results in a Priority Accuracy of 68.96\%, sufficient as a temporary measure.
...
\end{tabular}
\vspace{2pt}
\raggedright\scriptsize{$^*$Projected performances with noise applied during testing, from the validation data set. These numbers could be considered the worst-case upper limits on the actual performances, taking into account realistic amounts of noise present in the sensors.}
\end{table}

\textbf{3) Analysis:}

The network proves robust. Injecting $\sigma = 0.05$ (5\% noise) retains over 99\% of ``clean'' performance. F1@ 0.10 (10\%) for storm, earthquake, and flood are all above 0.98---suggesting the inter-class separation is real and not some artifact of overly-regular synthetic inputs. Though $\sigma = 0.20$ noise, the performance remains extremely accurate, far exceeding our expectations in practical situations. The resulting accuracy drops down to merely 87.3\%. Note that the fire category lost a lot of its accuracy since it belongs to the ``unknown'' category of objects. Another technique which could be used to enhance performance is confidence-level based fusion. Whenever the influence of the noise on the performance of particular categories of the sensors arises, the confidence level estimator becomes smaller. Therefore, the fusion layer acts as an adaptive noise filter.


\subsection{Training Convergence}

The training converges quickly. According to Table~\ref{tab:convergence}, the tri-fusion network achieves its minimal validation loss within 15 epochs.
...
Ablation provides a neat analysis of the role of each data source in the evaluation process. Satellite imagery is the most significant in terms of prioritization and severity since it does make sense as to why -- the image provides visual evidence of the damage caused, something that cannot be described using text. Sensor metadata provides the background information that cannot be gleaned from the satellite images, including the real event causing the risk such as rainfall, ground movement, and water levels. Finally, social media bridges the final gap by offering information on the individuals affected and their situation.
...
The very idea that modalities have to be lost in a percentage of 30 is of paramount importance for deployments. Let us take into consideration the first hour that is when the disaster occurs. It takes 30 to 60 minutes for the satellite images to arrive. Moreover, there might be no access to the sensors in the disaster zone. In the case of a disaster in an isolated area, there will be no social media either. Here comes the process of information flow, which depends on the stream.

\subsection{Contextualizing the ``Perfect'' Scores}

It might be worth noting a few caveats regarding those great F1 scores, however. There are certainly some factors that are in our favor: types of disasters have physically distinguishable sensor signals (wind/pressure vs magnitude/depth vs rain/elevation), the data for floods in Sri Lanka was synthetically created using rule-based signal generators and earthquake distributions were reasonably compact. Together, these factors help make the classification easier than it could ever hope to achieve in practice. At the same time, the experiments in Section~\ref{sec:noise_sensitivity} indicate the model is probably not memorizing the data, since it gets over 98\% F1 even with 10\% noise. Still, putting a machine learning algorithm on live sensor data will inevitably bring up problems that archival metadata can't: packet loss, battery effects, calibration differences between devices, etc. It's time for experiment three to be done on real data.

\subsection{Comparing Benchmarking of Priority Accuracy with Reality}

It appears almost too good to be true that we are managing to achieve up to 99.41\% priority accuracy, which is much better compared to other triage approaches like NG911 dispatch routing or severity detection using social media, with up to 85--87\% accuracy in spite of noisy input data. Yet, there must be some justification for this gap. In single-modality prioritization, a decision can be made based on the severity of input data; for instance, a tweet stating ``Looks pretty bad here.'' Our CoPrioritizer uses this tweet, along with IoT sensor spikes and satellite images showing destroyed buildings. Of course, it would make sense if all three types of input agreed, resulting in much simpler and more precise decisions. Thus, it could be assumed that our results are a product of our methodology and not due to any kind of bias toward carefully curated data.

\subsection{Positioning Relative to State of the Art}

Our sub-model performance rate when considering crisis using CrisisMMD is 86.39\%. This model outperforms the other attention-based models but lags behind 93.92--94.02\%, which was obtained by Mouzannar et al.~\cite{mouzannar2025guidedcross} with Guided Cross-Attention on LLaVA-generated captions.
A direct comparison is not quite fair, though, because our crisis model was designed to produce good 1024-dim embeddings for downstream fusion, not to push single-task accuracy as high as possible. Some of our architectural choices---the bidirectional cross-attention dimensionality, the confidence-weighted fusion---likely trade a bit of standalone accuracy for richer representations downstream.
...
\textbf{Archival metadata is not a live IoT feed.} The training data of the environmental sensors used are not real-time IoT data. It is important to note that the training data for the architectural design (the 32-dimension input layer and adaptive weighting output layer) are Kaggle metadata archives of historical weather data -- such as NOAA storm tracks, USGS earthquakes, etc. Notwithstanding that the architecture was made to cope with sensor constraints in the field, there are unique challenges in IoT implementations that archival data sets cannot account for, like sensor drift due to aging batteries, inconsistent sampling and calibration, etc. It is safe to say that when we refer to ``IoT'' in this paper, we mean architectural IoT only. We did not hurt MQTT transmission budgets, LoRaWAN duty cycles, and the other lovely names of IoT implementation in any way.

\textbf{Limitations of CPU-only training.} Although both IoT classifier and crisis sub-model were trained with Kaggle's GPUs (NVIDIA Tesla P100), the training of xBD segmentation and the tri-fusion was forced to use just CPUs due to the lack of GPU resources. Yet, being faced with such significant restrictions, the xBD model managed to yield a result of mIoU = 0.4276 after 30 epochs of training. The use of GPU training, the increase in batch size (up to 16--32), copy-paste augmentations and multi-scale training will lead to higher scores.

\textbf{Synthetic datasets based on rules.} For training IoT classifiers, rule-based syntheses have been made by means of the Sri Lanka Flood Risk Dataset~\cite{kaggle_srilanka_flood}. Likewise, the CORGIS Earthquake Dataset consists of synthetic samples whose distribution in terms of magnitude form rather narrow clusters thus facilitating generation of categorical separations~\cite{corgis_eq}. Although, according to Section~\ref{sec:noise_sensitivity}, these models show robustness to noise, the obtained F1-score results may be considered maximum possible values for synthetically generated data sets.
...
This paper presented a Tri-Modal Fusion architecture that brings together environmental sensor metadata, satellite damage imagery, and social media image--text pairs for real-time disaster intelligence, using pairwise cross-attention with adaptive gating to fuse the three streams. The system reaches 99.41\% priority classification accuracy and 0.0398 severity MAE, cutting error by 65.8\% compared to the crisis-only baseline. The ablation study across four modality configurations shows consistent improvement as each data source is added, and the model handles missing modalities without breaking. Noise injection also held up: the IoT model keeps over 98\% F1 on storm, earthquake, and flood at 10\% Gaussian noise, so the learned representations are doing more than memorizing clean inputs. Physically meaningful matching between disasters and sensors is ensured by the confidence-based fusion, while the modified gradient-based attention rollout algorithm [18], backed up by the four-level fall-back chain, enables the whole pipeline to maintain interpretability.

\subsection{Future Work}
\label{sec:future}

\begin{enumerate}
    \item \textbf{xBD next steps}: Training solely based on the CPU has reached its maximum limit where the model achieves an mIoU $\approx$ 0.4276. The only task left is to train on a GPU where larger batches (16--32) are used, longer durations (hundreds of epochs) are employed, multiresolution training is conducted with augmented test-time training, and augmented copy-pasting is performed in order to handle the long-tail of the damage classes. The target goal here is to attain an mIoU \textgreater 0.55, which is quite close to the range of 66--70\% attained by current state-of-the-art methods.
    \item \textbf{Sensor validation}: Can the model perform when live data from a sensor stream is fed to it? The next step would be to integrate live data streams from sensors such as USGS seismograph, NOAA atmospheric sensor, as well as IoT-based flooding sensor, and run the algorithm against real-world disturbance factors such as data packet loss, as well as sensor drift.
    \item \textbf{Multimodal temporal and spatial alignment}: It would be ideal if all the three modalities were aligned for the exact event in terms of both temporal and spatial dimensions, allowing us to test our multimodal fusion method without having to resort to artificial alignments.
    \item \textbf{Improving the crisis model}: Given recent innovations such as Guided Cross Attention~\cite{mouzannar2025guidedcross}, especially when combined with synthetic captioning, there are promising signs that it can bridge the gap between the existing CrisisMMD state-of-the-art models. The unanswered question at this point is whether fusion embedding quality can be maintained.
    \item \textbf{Field Testing}: The next step would be putting the system through field testing with first responders in actual disaster scenarios.
\end{enumerate}
