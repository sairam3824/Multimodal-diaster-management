...
Natural disasters in 2023 cost the world north of \$380 billion, and CRED's post-event reviews keep pointing at slow situational awareness as a preventable factor behind many of the deaths~\cite{cred2023}. Responders call those first hours the ``golden window,'' and how the window is handled tends to set the tone for the whole response. The awkward part: decision-makers inside that window usually have to work off fragmented bits of one source at a time. A seismograph trace without visual damage context. A tweet from the ground without any sensor data to back it. A satellite image with no one on the ground confirming what it shows.
...
\textbf{Environmental sensor metadata.} We assembled 63,527 samples from four public Kaggle sources: California Weather and Fire Prediction~\cite{kaggle_fire} (14,988 rows), Tropical Cyclone Tracks~\cite{kaggle_cyclone_tracks} (59,228), NOAA Atlantic Hurricanes~\cite{kaggle_storm_atl} (22,705), and Sri Lanka Flood Risk~\cite{kaggle_srilanka_flood} (25,000). One caveat worth spelling out: the Sri Lanka dataset is synthetic, produced by rule-based simulation for flood-risk prototyping. We used it to balance classes, but the perfect F1 numbers on flood-related categories partly reflect that synthetic regularity and should not be read as representative of noisy real-world sensors. Post preprocessing and subsampling, class counts come out as fire (4,971), storm (13,162), earthquake (18,394), flood (25,000), unknown (2,000). Features get rolled into a single 32-dim vector, with month and hour encoded through cyclic sin/cos.

\textbf{Data leakage prevention.} Derived risk scores such as \texttt{flood\_risk\_score} and \texttt{infrastructure\_score} were stripped out of the input features. They sit in the Sri Lanka dataset as target-correlated metadata, and feeding them to the model would be textbook leakage. What the model sees are primary physical variables: elevation, distance to river, rainfall, NDVI, and so on. The derived \texttt{flood\_risk\_score} is kept only for building the severity regression label. Table~\ref{tab:feature_mapping} lists the full feature-to-dimension mapping.
...
\textbf{Satellite imagery.} Post-disaster images come from xBD~\cite{gupta2019xbd} at $512 \times 512$, annotated with 4 damage classes (no-damage, minor, major, destroyed) spanning 12 events that include hurricanes, earthquakes, wildfires, flooding, tsunami, and volcanic eruptions. Once filtered the pool lands at 2,684 samples, split 80/20 into 2,147 train and 537 validation.
...
\caption{IoT model confusion matrix, raw counts on the left and normalized on the right. Storm, earthquake, and flood come through with clean separation; the bulk of the remaining errors sits in the fire / unknown pair.}
\label{fig:iot_cm}
\end{figure}
...
\caption{Severity regression results. Left panel plots predicted versus true severity ($R^2 = 0.745$); right panel shows the residuals, with MAE = 0.0452 and RMSE = 0.1188.}
...
One reasonable concern about the perfect F1 on storm, earthquake, and flood is whether the model is just memorizing clean or synthetically regular inputs. To check, we run controlled noise injection on the IoT model at inference.

\subsubsection{Methodology}

We add zero-mean Gaussian noise to the 32-dim normalized sensor vectors at $\sigma \in \{0.05, 0.10, 0.20\}$, i.e.\ 5\%, 10\%, and 20\% of the $[0,1]$ feature range, then clamp the perturbed values back into $[0,1]$. The idea is to approximate what real sensor deployments toss at you: drift, calibration error, environmental interference, and the rest.

\subsubsection{Results}

Results are laid out in Table~\ref{tab:noise_sensitivity}.
...
The network \textbf{degrades gracefully}. At $\sigma=0.05$ (5\% noise) over 99\% of clean performance is retained. At 10\%, F1 for storm, earthquake, and flood all sit above 0.98---evidence that the inter-class separation is real rather than an artifact of synthetic regularity. Even at 20\% noise, which is past anything you would routinely see in the field, overall accuracy still reaches 87.3\%. Fire drops the most, and that was expected given its overlap with the ``unknown'' class.

Confidence-weighted fusion contributes here too. When noise corrupts one sensor group, the per-group confidence estimators learn to dial that group down, which effectively turns the fusion layer into an implicit noise filter.

\subsection{Training Convergence}

Convergence is fast. Table~\ref{tab:convergence} shows the tri-fusion model reaching its best validation loss by epoch 15.
